...
public class TagUtils {
	...
	private TagUtils() {
		BungeeChannel.getInstance().getProxy().getScheduler().schedule(BungeeChannel.getInstance(), () -> playerData.keySet().stream().filter(player -> BungeeChannel.getInstance().getProxy().getPlayer(player) == null).forEach(player -> playerData.remove(player).clear()), 180, 180, TimeUnit.SECONDS);
	}
